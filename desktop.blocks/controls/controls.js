...
modules.define('controls', ['i-bem__dom', 'BEMHTML', 'tabs', 'select-autocomplite', 'list', 'jquery'], function(provide, BEMDOM, BEMHTML, tabs, formSelect, listMonth, $, controls) {
	provide(controls.decl({modName: 'events', modValue: true}, {
		...
		onSetMod: {
			'js': {
				'inited': function() {
					...
					this._blockList = this.findBlockOutside('page').findBlockInside('abstract-section-list');
					...
					this._mountTitle = this.findBlockOutside('page').findBlockInside('content').elem('title');
					this._selectCity = {};
					this._selectYear = {};
					this._selectCity.currentValue = this.findBlockOutside('page')
						.findBlockInside('tabs').getCurrentValue();
		 ...
		 * @private
		 */
		_changeControls: function() { /* основной смысл спрятан тут - добавление полученного через аджакс контента */
			var self = this;

			if (this._bemJSON) {
				BEMDOM.append(this._blockList.domElem[0], BEMHTML.apply(this._bemJSON)); /* добавляет абстрактный лист */
				this.filter(false, this._selectMonth.currentValue);
		...
		getEvents: function(year, month) { /* аджакс запрос событий */
			...
			/*
			 ...
			 type: 'POST',
			 url: '',
			 data: {type: 'request', year: year, month: month},
			 dataType: 'json',
