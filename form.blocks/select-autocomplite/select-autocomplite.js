			...
			onSetMod: {
				'js': {
					...
					'inited': function() {
						...
						//	realWidth = 0,
						//	borderWidth = 0;
						//borderWidth = parseInt( $('.select-autocomplite-input').css('border-right-width'), 'px') * 2;
						//$('.select-autocomplite-list__item').each(function(){
						//	realWidth = Math.max(realWidth,  $(this).outerWidth());
						//});
						//maximumWidth = realWidth + borderWidth;
                        //
						////console.log('maximum width: ', maximumWidth);
                        //
						//$('.select-autocomplite').css('width', maximumWidth );
